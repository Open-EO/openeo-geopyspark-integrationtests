...
def test_catboost_training(auth_connection: openeo.Connection, tmp_path, auto_title):
    ...
    model: MlModel = model.save_ml_model()
    job: BatchJob = model.create_job(title=auto_title + " train", job_options={"executor-cores":1})
    assert job.job_id
    job.start_job()
...
def test_filter_by_multiple_tile_ids(auth_connection, auto_title, job_options, expected_derived_from_link_style):
    ...
    cube = auth_connection.load_collection(
        "SENTINEL2_L2A",
        spatial_extent={"west": 4.415, "south": 51.42, "east": 4.461, "north": 51.46},
        temporal_extent=["2024-04-24", "2024-04-25"],
        bands=["B02"],
        properties=[
            openeo.collection_property("tileId").is_one_of(tile_ids),
        ],
    )

    job = cube.save_result("GTiff").create_job(
        title=auto_title,
